...
void processing(int row, int column) {
	...
	if (digitalRead(IR_barrier_signal_pin) == 0) {
		mosfet.digitalWrite(0, row, CATODES_PASSIVE_STATE);  // catodes  
		mosfet.digitalWrite(1, column, ANODES_ACTIVE_STATE);    // anodes high
		flag_timeout = 0;
		time_start = millis();
		while ((digitalRead(IR_barrier_signal_pin) == 0 ) && (flag_timeout == 0)){
			if (millis() > time_start + IR_TIMEOUT_MS)  {
				flag_timeout = 1;
			}
		}   
	} else 	{
